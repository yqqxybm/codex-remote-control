...
The relay requires `CRC_RELAY_ACCESS_TOKEN` at startup unless an isolated local run explicitly sets `CRC_RELAY_ALLOW_UNAUTHENTICATED=1`. It closes sockets that do not send an authenticated `hello` promptly and caps WebSocket payload size before application parsing while still allowing large encrypted session reads.
